...
const DonorProfile = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await api.put(`/donor/update/${user._id}`, filteredData, {
        headers: {
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/donor/delete/${user._id}`);
  ...
  const handlePrivacyToggle = async () => {
    try {
      const newPrivacy = user.privacy === 0 ? 1 : 0;
      await api.patch(
        "/donor/privacy",
        { privacy: newPrivacy },
        {
      ...
      setUser((prev) => ({ ...prev, privacy: newPrivacy }));
      toast({
        title: `Profile is now ${newPrivacy === 0 ? "Visible" : "Hidden"}`,
        status: "success",
        duration: 3000,
        isClosable: true,
        position: "top",
    ...
    <Box maxW="1200px" mx="auto" p={{ base: 4, md: 6 }}>
      <Flex direction={{ base: "column", md: "row" }} gap={6}>
        ...
        >
          ...
          <Tag
            colorScheme={user.validation === 0 ? "green" : "red"}
            size="lg"
            mt={2}
            borderRadius="full"
          >
            <TagLeftIcon
              as={user.validation === 0 ? FiCheckCircle : FiXCircle}
            />
            <TagLabel>
              {user.validation === 0 ? "Verified" : "Not Verified"}
            </TagLabel>
          </Tag>
          ...
          <FormControl display="flex" alignItems="center" mt={6} ml={8}>
            <Switch
              id="privacy-toggle"
              isChecked={user.privacy === 0}
              onChange={handlePrivacyToggle}
              colorScheme="red"
              mr={2}
            />
            <FormLabel htmlFor="privacy-toggle" mb="0" fontSize="sm">
              {user.privacy === 0
                ? "Profile Visible to Requesters"
                : "Profile Hidden from Requesters"}
            </FormLabel>
          </FormControl>
